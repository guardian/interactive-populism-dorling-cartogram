...
function makeStacked(rectWidth, rectHeight, domainX, domainY, data, group, populists = ['leftshare', 'othershare', 'rightshare'])
{
  let x = d3.scaleTime()
  ...
  if(group.node().getAttributeNode("class").value == "area-group-lines")
  {
    ...
    year1.attr("transform", "translate("+ 0 + "," + (rectHeight + 15) + ")")
    year2.attr("transform", "translate("+ (x(new Date(2008, 0, 1)) - (year2.node().getComputedTextLength() / 2)) + "," + (rectHeight + 15) + ")")
    year3.attr("transform", "translate("+ (x(new Date(2018, 11, 30)) - year3.node().getComputedTextLength()) + "," + (rectHeight + 15) + ")")
